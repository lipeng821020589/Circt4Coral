...
func.func @test_stripmine_vadd(%a: i32, %b: i32) -> i32 {
  ...
  // CHECK: coralnpu.vadd stripmine = 4 {{.*}} : (i32, i32) -> !coralnpu.vreg<e32, m1>
  %r = coralnpu.vadd %a, %b : (i32, i32) -> !coralnpu.vreg<e32, m1>
  %s = coralnpu.vredsum %r : (!coralnpu.vreg<e32, m1>) -> i32
...
func.func @test_stripmine_vsub(%a: i32, %b: i32) -> i32 {
  ...
  // CHECK: coralnpu.vsub stripmine = 4 {{.*}} : (i32, i32) -> !coralnpu.vreg<e32, m1>
  %r = coralnpu.vsub %a, %b : (i32, i32) -> !coralnpu.vreg<e32, m1>
  %s = coralnpu.vredsum %r : (!coralnpu.vreg<e32, m1>) -> i32
  coralnpu.return %s : i32
}
...
func.func @test_stripmine_vdot(%a: i32, %b: i32) -> i32 {
  ...
  // CHECK: coralnpu.vdot stripmine = 4 {{.*}} : (i32, i32) -> !coralnpu.vreg<e32, m1>
  %r = coralnpu.vdot %a, %b : (i32, i32) -> !coralnpu.vreg<e32, m1>
  %s = coralnpu.vredsum %r : (!coralnpu.vreg<e32, m1>) -> i32
  coralnpu.return %s : i32
}
...
func.func @test_stripmine_already_set(%a: i32, %b: i32) -> i32 {
  ...
  // CHECK: coralnpu.vadd stripmine = 2 {{.*}} : (i32, i32) -> !coralnpu.vreg<e32, m1>
  %r = coralnpu.vadd stripmine = 2 %a, %b : (i32, i32) -> !coralnpu.vreg<e32, m1>
  %s = coralnpu.vredsum %r : (!coralnpu.vreg<e32, m1>) -> i32
  coralnpu.return %s : i32
}
